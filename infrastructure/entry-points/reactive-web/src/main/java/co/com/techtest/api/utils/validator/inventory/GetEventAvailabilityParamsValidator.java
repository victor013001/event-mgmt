...
@UtilityClass
public class GetEventAvailabilityParamsValidator {

    public static Mono<List<ErrorDetail>> validateAvailabilityParam(String eventId) {
        return isValidNotNullAndNotBlank(eventId)
                .map(GetEventAvailabilityParamsValidator::validations);
    }

    private static List<ErrorDetail> validations(Boolean isValid) {
        List<ErrorDetail> errors = new ArrayList<>();
        validateField(isValid, errors, TechnicalMessageType.ERROR_MS_INVALID_EVENT_ID);
        return errors;
    }
}
